...
- Mark `setlocale` and `TextDomain::init()` as `unsafe` because calling
    `setlocale` from multiple threads can lead to memory corruption. See updated
    docs for details on how to call those functions safely (#64)
    (Alexander Batischev)
